...
| 5 | `types/database.types.ts` | 2 (quality) | The file is **hand-authored** but mixes in custom exports (`MealSlot`, `RecipeSourceKind`, `UpdateTables`, …); running `npm run db:types` (`supabase gen types`) overwrites the whole file and deletes those, breaking ~19 importers. Best practice: let the generator own `database.types.ts`, move custom helpers to a separate `types/database.ts` (`export * from "./database.types"` + enum aliases via `Enums<>`), repoint the 19 imports. Risk: the generated `Database` shape may differ subtly from the hand-authored one → surfacing type errors, so do it deliberately with typecheck, not mid-module. | Module 4 (schema change for `entra_oid`) | Open — deferred to Module 9 (DB layer for Neon) |
