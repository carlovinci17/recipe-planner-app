-- ADR-002 / Lesson 3.4: bridge the shopping-list RPC to app_uid() so it runs on
-- the Drizzle connection (no auth.uid() there). The 7-day wrapper delegates to
-- the range function, so only the range function changes — its sole edit is
-- `_uid := public.app_uid()` (was auth.uid()). app_uid() coalesces to auth.uid()
